...
int32 UInventoryComponent::CalcualteNumberForFullStack(UItemBase* StackableItem, int32 InitialRequestedAddAmount)
{
...
FItemAddResult UInventoryComponent::HandleNonStackableItems(UItemBase* InputItem)
{
	...
	if (FMath::IsNearlyZero(InputItem->GetItemSingleWeight()) || InputItem->GetItemSingleWeight() < 0)
	{
		return FItemAddResult::AddedNone(FText::Format(
			FText::FromString("Coult not add {0} to the inventory. Item has invalid weight value."), InputItem->TextData.Name));
	}

	if (InventoryTotalWeight + InputItem->GetItemSingleWeight() > GetWeightCapacity())
	{
		return FItemAddResult::AddedNone(FText::Format(
			FText::FromString("Coult not add {0} to the inventory. IItem would overflow weight limit."), InputItem->TextData.Name));
	}
	...
	{
		return FItemAddResult::AddedNone(FText::Format(
			FText::FromString("Coult not add {0} to the inventory. All Inventory slots are full."), InputItem->TextData.Name));
	}

	AddNewItem(InputItem, 1);
	return FItemAddResult::AddedAll(1, FText::Format(
		FText::FromString("Successfully added {0} {1} to the inventory."), InputItem->TextData.Name));
...
int32 UInventoryComponent::HandleStackableItems(UItemBase* ItemIn, int32 RequestedAddAmount)
{
	...
	while (ExistingItemStack)
	{
		...
		const int32 AmountToMakeFullStack = CalcualteNumberForFullStack(ExistingItemStack, AmountToDistribute);
		// calculat how many of the AmountToMakeFullStack can actually be carried based on weight capacity
		const int32 WeightLimitAddAmount = CalculateWeightAddAmount(ExistingItemStack, AmountToMakeFullStack);

		// as long as the remaining amount of the item does not overflow weight capacity
		if (WeightLimitAddAmount > 0)
		{
			// adjust the existing items satck quantity and inventory total weight
